...
from src.schemas.experimental import PageText, SectionSpan
...
def _fallback_sections(pages: list[PageText]) -> list[SectionSpan]:
    ...
    spans = []
    if total >= 1:
        spans.append(SectionSpan(name="abstract", page_start=1, page_end=1, line_start=0, weight=SECTION_WEIGHTS["abstract"]))
    if total >= 3:
        spans.append(SectionSpan(name="introduction", page_start=2, page_end=min(3, total), line_start=0, weight=SECTION_WEIGHTS["introduction"]))
    if total >= 4:
        spans.append(SectionSpan(name="methods", page_start=3, page_end=total, line_start=0, weight=SECTION_WEIGHTS["methods"]))
