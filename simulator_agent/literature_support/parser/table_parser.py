...
Identifies tables whose columns map to experimental conditions (T, P, τ, Φ,
species concentrations) and extracts rows as structured data.

Handles common PyMuPDF text-extraction artifacts:
- Greek symbols rendered as ASCII: τ→t, Φ→F
...
_HEADER_RULES: list[tuple[re.Pattern, str, str | None]] = [
    ...
    # Residence time — τ often extracted as bare 't' by PyMuPDF
    (re.compile(r'^[\u03c4\u03c4]$'), 'residence_time', 's'),          # τ
    (re.compile(r'^t$'), 'residence_time', 's'),                        # ASCII fallback
    (re.compile(r'^(?:\u03c4|tau|residence)', re.I), 'residence_time', 's'),

    # Equivalence ratio — Φ often extracted as bare 'F' by PyMuPDF
    (re.compile(r'^[\u03a6\u03c6\u0278]$'), 'equivalence_ratio', None), # Φ, φ, ɸ
...
def _looks_like_range(a: float, b: float, column: TableColumn) -> bool:
    ...
    """
    # If exponent side has a decimal, it's not scientific notation
    # (handled implicitly — both are already floats)
